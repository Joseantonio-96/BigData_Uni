...
# Set the environment for sockets parallelization
registerDoParallel()
...
clusterExport(cl, list("data_new", "createCluster"))
cl_withinss = parLapply(cl, k_clust, createCluster)
# stop
stopCluster(cl)
...
Custer_avgs <- data_clusters_selected %>% group_by(cluster) %>% summarise(mean = mean(price))
