...
cl_withinss <- foreach::foreach (i = k_clust, .combine = list, .multicombine=TRUE) %dopar% {createCluster(i)}
stopImplicitCluster()
cl_withinss <- as.numeric(cl_withinss)
...
NUM_CLUSTERS = 4
kmeansModel <-kmeans(data_new, NUM_CLUSTERS, iter.max = 100)
